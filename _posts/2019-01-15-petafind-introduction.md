...
Yocto 是面向大型工程设计的，它期望能让多个厂商的合作更方便，比如 meta-layer 的设计，每个厂商都能通过 bbappend 文件在自己的 meta-layer 中为大家共同协作的软件包添加新的 patch。获得这种便利性的情况下其实也增加了整个项目的复杂度。也就是说，找菜谱几乎变成了一个寻宝游戏，菜谱散落在世界各地（安装目录中的各个 meta-layer 以及 PetaLinux 工程中的 meta-user layer），要得到最终完整的菜谱，就要把各个大陆全都翻查一遍，简直是个体力活。
